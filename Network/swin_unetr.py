...
class SelectiveNetSwinTs(nn.Module):
    """
    Swin UNETR based on: "Hatamizadeh et al.,
    Swin UNETR: Swin Transformers for Semantic Segmentation of Brain Tumors in MRI Images
    <https://arxiv.org/abs/2201.01266>"
    ...
    def __init__(
        ...
        print(depths,'this is depths of model')
        self.avgpool = nn.AdaptiveAvgPool3d((1, 1, 1))
        self.classifier = nn.Linear(self.final_features, num_class)

        self.selector = torch.nn.Sequential(
        torch.nn.Linear(self.final_features, self.final_features),
        torch.nn.ReLU(True),
        #torch.nn.BatchNorm1d(self.dim_features),
        #replace by instance norm
        torch.nn.InstanceNorm1d(self.final_features),
        torch.nn.Linear(self.final_features, 1),
        torch.nn.Sigmoid()
    )
        self.aux_classifier = torch.nn.Sequential(
            torch.nn.Linear(self.final_features, num_class)
        )
    ...
    def forward(self, x_in):
        ...
        pre_out = self.classifier(logits)
        select_out = self.selector(logits)
        aux_out = self.aux_classifier(logits)
