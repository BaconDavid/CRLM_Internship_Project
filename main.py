...
def main(data_path,label_path,save_path,epochs,num_class,mode='train'):
    ...
    stratify_kfold = StratifiedKFold(n_splits=2,shuffle=True,random_state=42)
    ...
    if mode == 'train':
        ...
        for fold,(train_index,vali_index) in enumerate(stratify_kfold.split(images_lst,labels_lst)):
            ...
            dataset = Image_Dataset(image_files=train_images,labels=train_labels,transform_methods=transform_param['transform_methods'],data_aug=True,label_name=None)
            dataloader = Data_Loader(dataset=dataset,batch_size=1,num_workers=0).build_train_loader() 

            #set scheduler,optimizer parameters

            optimizer_param = {"lr":0.001}
            scheduler_param = {"step_size":100,"gamma":0.1}
            ...
            for epoch in range(epochs):
                ave_loss,y_pred,AUC,accuracy,F1,four_rate_dic = train_loop(model,dataloader,epoch,'cpu',num_class,optimizer_fun,scheduler_fun,loss_fun,visual_im=True,visual_out_path=args.visual_out_path)
                tr_results.store_results(tr_results.df_results(four_rate_dic,AUC,accuracy,F1,ave_loss,epoch))
            

        ###########validation##############
                model = build_model('resnet10',num_class)
                dataset = Image_Dataset(image_files=vali_images,labels=vali_labels,transform_methods=transform_param['transform_methods'],data_aug=True,label_name=None)
                dataloader = Data_Loader(dataset=dataset,batch_size=1,num_workers=0).build_vali_loader() 
                loss_fun = Loss().build_loss()

                #save results
                val_results = SaveResults(save_path + f"fold{fold}/",'vali')

                ave_loss,y_pred,AUC,accuracy,F1,four_rate_dic = Validation_loop(model,dataloader,'cpu',num_class,loss_fun,visual_im=True,visual_out_path=args.visual_out_path)
